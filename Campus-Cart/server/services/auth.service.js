...
class AuthService {
    ...
    if (user) {
      try {
        ...
        if (process.env.ENABLE_EMAILS === 'true') {
          ...
          console.log('Reset Token:', fpSalt);
          console.log('Reset URL: http://localhost:3000/reset-password?token=' + fpSalt);
        }
        //await mail.forgotPasswordMail(fpSalt, email);
      } catch (error) {
        throw new ErrorHandler(error.statusCode, error.message);
  ...
  async signToken(data) {
    try {
      // 临时硬编码SECRET测试
      const secret = process.env.SECRET || 'course-cart-fallback-secret-for-testing-12345';
      console.log('🔑 使用JWT密钥:', secret.substring(0, 10) + '...');
  ...
  async signRefreshToken(data) {
    try {
      // 临时硬编码REFRESH_SECRET测试
      const refreshSecret = process.env.REFRESH_SECRET || 'course-cart-refresh-fallback-secret-12345';
